...
### Known limits
- **The compression level does nothing on the WebM route** ([#25](https://github.com/ogfrench/frogConvert/issues/25)). `-crf` is inert for libvpx in this core: `-crf 23`, `-crf 32`, and both again with `-b:v 0` all produce byte-identical output, while `-b:v 2M` moves it fivefold. So a WebM comes back at libvpx's default bitrate whichever level was chosen. Found while fixing #23 and filed rather than folded in, because it changes output for every WebM user.
