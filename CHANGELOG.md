...
- **The file manager says what it does.** *Remove all* is now **Clear** in the shared Files modal, and the Converter's refresh button is **Replace all files** rather than "Replace file" - it always discarded the whole queue, singular label notwithstanding. Compress's identical-looking button keeps "Add more files", because it appends.
...
- **WebM picks encoders this core can run.** `libvpx-vp9` and `libopus` both die with a memory-access fault on a two-second clip, and ffmpeg selects exactly that pair for a `.webm` output. Both are pinned to encoders that complete. This does not close [#23](https://github.com/ogfrench/frogConvert/issues/23) - short clips convert, a 20-second 1080p recording still fails, and the remaining cause looks like memory rather than codec choice.

### Known limits
- **MP4 → WEBM still fails on longer clips** ([#23](https://github.com/ogfrench/frogConvert/issues/23)). Codec pinning fixed short ones; a 20-second 1080p recording does not complete.
