...
### Known limits
- The PDF Editor's **edits** (merge, organize, watermark, extract) cannot be cancelled once started; only the optional compression step that follows them can be skipped. They are local pdf-lib work rather than a network fetch, but a watermark across several hundred pages still leaves you on a spinner. Tracked in #21.
